...
const firstLetters = (arr) => {
    // Solution code here...
    return arr.map(elemnt=>{
      return elemnt.charAt(0)
    })
  };
...
const findHappiness = (arr) => {
    ...
    let result=[];
     arr.forEach(elemnt=>{
      if(elemnt.includes(":)")) result.push(elemnt);
    })
    return result
  };
/* ------------------------------------------------------------------------------------------------
...
const onlyOddChars = (str) => {
    ...
    for(let i=0;i<arr.length;i++){
      if(!isNaN(arr[i])){
         if(str.charCodeAt(i)%2!==0) result.push(str.charAt(i))
      }else if(str.charCodeAt(i)%2==0) result.push(str.charAt(i))
    }
...
const findEvery = (arr, target) => {
    let result = arr.map(val => val.includes(target));
    return result
};
