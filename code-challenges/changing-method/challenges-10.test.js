...
const count = (target, input) => {
    // Solution code here...
    let counter =0;
    let result  = input.map((acummlate,val)=>{
      console.log(acummlate)
      for(let i=0;i<acummlate.length;i++){
      if(acummlate[i] ==target) counter++
      }
    })
    return counter
  };
...
let findMaleAndFemale = (data) => {
  ...
  let str =''
  let arr=[]
  let array= data.map((val,idx)=>{
    // console.log(val)
    if(val.gender ==='male' || val.gender ==='female')  arr.push(val.name)
  })
  arr.forEach(elemnt=>{
    // console.log(elemnt)
    str = str +`${elemnt} and `
...
let findShortest = (data) => {
  ...
  let str;
  let array= data.map((val,idx)=>{
    if(val.height<100)  str=`${val.name}`
  })
   return str
};
